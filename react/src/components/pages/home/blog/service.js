...
export function blogListRequest(params) {
...
export function blogAddRequest(params) {
    return dispatch => (
        new Promise((resolve, reject) => {
            Http.post('api/v1/articles', transformRequest(params))
                .then(res => {
...
export function blogUpdateRequest(params) {
    return dispatch => (
        new Promise((resolve, reject) => {
            Http.patch(`api/v1/articles/${params.id}`, transformRequest(params))
                .then(res => {
...
export function blogRemoveRequest(id) {
    return dispatch => {
        Http.delete(`api/v1/articles/${id}`)
            .then(() => {
                dispatch(articleActions.remove(id))
...
export function blogEditRequest(id) {
    return dispatch => {
        Http.get(`api/v1/articles/${id}`)
            .then((res) => {
                dispatch(articleActions.add(transformResponse(res.data)))
...
export function blogFetchRequest(slug) {
    return dispatch => {
        Http.get(`api/v1/articles/published/${slug}`)
            .then((res) => {
                dispatch(articleActions.add(transformResponse(res.data)))
